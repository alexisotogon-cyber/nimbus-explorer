    ...
    const screenGrounding = screenContext
      ? `\n\nCONTEXTO DE INTERFAZ ACTUAL (sirve para resolver "esto", "esta sección" y "lo que veo"):\n${JSON.stringify({
        ...
        (screenContext.findingsList
          ? `\n\nADVERTENCIA SOBRE NUMERACIÓN: la lista findingsList arriba es la única fuente válida para "hallazgo 1/2/3" o "el primero/segundo" — NUNCA la sustituyas por tu propia priorización interna, ni siquiera cuando el hallazgo en esa posición tenga $0 de ahorro (hallazgos de visibilidad, como gasto en IA sin desglosar, cuentan y ocupan su posición igual que cualquier otro). "Hallazgo 1" significa literalmente \`findingsList[0].id\`, sin excepción ni criterio propio de qué "parece" más relevante. El tablero agrupa hallazgos en varias secciones (ej. "Grandes proyectos") que cada una reinicia su propia numeración desde 1, así que un mismo número puede referirse a hallazgos distintos según la sección que el usuario esté viendo. Si el hallazgo que el usuario describe (por número o de forma vaga) no coincide claramente con esta lista, o si el número podría corresponder a más de un hallazgo real, PREGUNTA el título exacto antes de responder — no asumas cuál es.`
          : "")
      : "";
